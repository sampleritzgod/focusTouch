...
| `GET` | `/api/v1/focus/sessions/active` | None. | User may have at most one active session. | Bearer token required. | `200` with `{ "focusSession": FocusSession or null }`. | Standard error with `UNAUTHORIZED`. | `200`, `401`, `500` |
